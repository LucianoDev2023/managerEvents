...
export default function EditMyParticipationScreen() {
  ...
  useEffect(() => {
    const fetchParticipation = async () => {
      ...
      try {
        const guest = await getGuestParticipation(id, userEmail);
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      await new Promise((res) => setTimeout(res, 400)); // simula pequena latência
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await updateParticipation(id, userEmail, { family });
    ...
    >
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.inputRow}>
          ...
          />
          <Pressable
            style={[styles.addButton, { opacity: adding ? 0.6 : 1 }]}
            onPress={handleAdd}
            disabled={adding}
          >
            {adding ? (
